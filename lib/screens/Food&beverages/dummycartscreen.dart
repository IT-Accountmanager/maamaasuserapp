...
// //
// //           if (ok) {
// //             food_Authservice.capturePayment(paymentId: pid, amount: amount)
...
//   PaymentOverlayState _overlayState = PaymentOverlayState.none;
//
//   // ── Socket/HTTP arbitration ────────────────────────────────────────────────
//   // _socketVersion increments every time a real socket event is applied.
//   // Any HTTP fetchCart response checks if the version changed during its
//   // await — if it did, the socket already delivered fresher data so the
//   // HTTP result is silently dropped instead of overwriting the UI.
//   bool _isLoadingCart = false;
...
//   //   _scrollController = ScrollController();
//   //
//   //   // FIX Bug 1: _initCartSocket subscribes FIRST so no events are missed,
//   //   // then _loadAllData does the one-time HTTP fetch. After that, only the
//   //   // socket updates the cart — no more HTTP polling.
//   //   _initCartSocketFirst();
//   //   _loadAllData();
//   // }
//   @override
//   void initState() {
...
//   }
//
//   // ── Initial load: HTTP once, then socket owns all cart updates ────────────
//   // Key rules:
//   //   1. _isLoadingCart=true before await → socket events are buffered.
//   //   2. Snapshot the socketVersion BEFORE the await. If it changed by the
//   //      time fetchCart returns it means the socket already delivered fresher
//   //      cart data — drop the HTTP cart result and don't overwrite the UI.
//   //   3. wallet + ads are never pushed by the socket, so they always apply.
//   //   4. _flushPendingSocketUpdates() in finally applies any buffered event.
//
//   // Future<void> _loadAllData() async {
//   //   _isLoadingCart = true;
//   //   setState(() => isLoading = true);
//   //
//   //   // Snapshot version BEFORE the HTTP call goes out
//   //   final versionAtStart = _socketVersion;
//   //
//   //   try {
//   //     final results = await Future.wait([
//   //       food_Authservice.fetchCart(),           // index 0
//   //       subscription_AuthService.fetchWallet(), // index 1
//   //       promotion_Authservice.fetchcampaign(),  // index 2
//   //     ]);
//   //
//   //     if (!mounted) return;
//   //
...
//   //
//   //     setState(() {
//   //       // ✅ Only apply the HTTP cart if NO socket event arrived while we waited.
//   //       // If _socketVersion changed it means the socket already updated cartData
//   //       // with fresher server data — using the HTTP result would regress the UI.
//   //       if (_socketVersion == versionAtStart) {
//   //         cartData = cart;
//   //         if (cartData?.hasAnyScheduledItem ?? false) {
//   //           _orderType = 'schedule';
//   //         }
//   //         debugPrint("✅ HTTP cart applied (no socket arrived during fetch)");
//   //       } else {
//   //         debugPrint(
//   //           "⚠️ HTTP cart DROPPED — socket already delivered fresher data "
//   //               "(v$versionAtStart → v$_socketVersion)",
//   //         );
//   //       }
//   //
//   //       // wallet and ads always apply — socket never pushes these
//   //       wallet = walletData;
//   //       homepageAds = ads
//   //           .where(
//   //             (c) =>
...
//
//     // ✅ Get userId and subscribe socket BEFORE awaiting HTTP
//     final prefs = await SharedPreferences.getInstance();
//     userId = prefs.getInt('userId') ?? 0;
//     WebSocketManager().subscribeUserCart(userId, _updateCartFromSocket);
//
...
//   }).toList();
//
//   // ── FIX Bug 1: subscribe before _loadAllData so no early events are lost ──
//   // void _initCartSocket() async {
//   //   final prefs = await SharedPreferences.getInstance();
//   //   userId = prefs.getInt('userId') ?? 0;
//   //   WebSocketManager().subscribeUserCart(userId, _updateCartFromSocket);
//   // }
//
//   Future<void> _initCartSocketFirst() async {
//     final prefs = await SharedPreferences.getInstance();
//     userId = prefs.getInt('userId') ?? 0;
//     if (userId == 0) return;
//
//     // Subscribe immediately — socket may fire before HTTP completes
//     // _updateCartFromSocket handles cartData == null case
//     WebSocketManager().subscribeUserCart(userId, _updateCartFromSocket);
//   }
//
//   // void _updateCartFromSocket(Map<String, dynamic> data) {
//   //   // Deduplicate: server sends each event twice. Drop the second identical fire.
//   //   final firstItem = (data['cartItems'] as List?)?.firstOrNull;
//   //   final eventKey = '${data['cartId']}_${firstItem?['updatedAt']}';
//   //   final items = data['cartItems'] as List? ?? [];
//   //   final itemsSig = items
//   //       .map((i) => '${i['itemId']}:${i['quantity']}')
//   //       .join(',');
//   //   // final eventKey = '${data['cartId']}_$itemsSig';
//   //
//   //   if (eventKey == _lastSocketEventKey) {
//   //     debugPrint("🔁 Duplicate socket event ignored");
//   //     return;
//   //   }
//   //   _lastSocketEventKey = eventKey;
//   //   Future.delayed(
...
//   //
//   //   if (_isLoadingCart) {
//   //     // HTTP fetch is in-flight — buffer this event so it is applied AFTER
//   //     // the HTTP load finishes (via _flushPendingSocketUpdates).
//   //     debugPrint("⏳ HTTP in-flight → buffering socket event");
//   //     _pendingSocketUpdates.clear();
//   //     _pendingSocketUpdates.add(data);
//   //     return;
//   //   }
//   //
//   //   if (cartData == null) {
//   //     // cartData not yet initialised (very early socket event before
//   //     // _loadAllData has returned). Apply immediately using fromJson so the
//   //     // user sees real data instead of a blank screen.
//   //     debugPrint("⚡ cartData null → applying socket as initial cart");
//   //     try {
//   //       cartData = CartModel.fromJson(data);
//   //       if (mounted) setState(() {});
//   //     } catch (e) {
//   //       debugPrint("❌ CartModel.fromJson failed: $e — buffering instead");
...
//
//     // ✅ Dedup by updatedAt of first item — catches the server's double-fire
//     final firstItem = socketItems.isNotEmpty ? socketItems.first as Map : null;
//     final eventKey = '${data['cartId']}_${firstItem?['updatedAt']}';
...
//   //   if (!mounted) return;
//   //
//   //   // ✅ Increment version BEFORE setState so any concurrent HTTP fetchCart
//   //   // that checks _socketVersion after its await will see the change and
//   //   // drop its stale result instead of overwriting what the socket just set.
//   //   _socketVersion++;
//   //
//   //   setState(() {
//   //     cartData!.cartItems = items.map((json) {
...
//   }
//
//   // ── FIX Bug 5: gate socket during HTTP call so optimistic qty is not
//   //   overwritten by a stale socket event that arrives before the server
//   //   echoes back the updated cart via the socket. ──────────────────────────
//   Future<void> changeQuantity(CartItem item, int newQty) async {
//     final old = item.quantity;
//
