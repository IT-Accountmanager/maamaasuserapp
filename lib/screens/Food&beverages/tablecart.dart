...
import 'package:maamaas/screens/Food&beverages/tablemenu.dart';
  ...
  Future<void> placeOrder() async {
    ...
    try {
      ...
      /// 🔵 ONLINE PAYMENT (RAZORPAY)
        ...
        razorpay.onSuccess = (response) async {
          ...
          if (!captured) {
            AppAlert.error(
              context,
              "❌ Payment capture failed. If amount was debited, refund will be processed within 3–5 working days.",
            );
            return;
          }

          /// 2️⃣ IF CAPTURE SUCCESS → PLACE ORDER
          await _callOrderApi(
            userId: userId,
      ...
      /// 🟣 WALLET PAYMENT
        ...
        if (walletType == null) {
          AppAlert.error(context, "⚠️ Please select a sub-wallet");
          return;
        }
