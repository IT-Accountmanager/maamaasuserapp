...
class _ConfirmedListCardState extends State<ConfirmedListCard> {
  ...
  // Future<void> _handleArrivalTap() async {
  //   setState(() => _loading = true);
  //   final distance = await _getDistanceFromRes_getDistanceFromRestauranttaurant();
  //   setState(() => _loading = false);
  //   if (distance == null) {
  ...
  Future<void> _handleArrivalTap() async {
    ...
    try {
      ...
      if (mounted) {
        AppAlert.error(context, 'Failed to confirm arrival');
      }
    } finally {
      if (mounted) {
        setState(() => _loading = false);
...
//             disabled: locked,
//
//             onTap: () => _showArrivalConfirmation(context),
//           ),
//         ),
//         const SizedBox(width: 8),
//
