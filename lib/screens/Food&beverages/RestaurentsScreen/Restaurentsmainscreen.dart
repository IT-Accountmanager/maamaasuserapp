...
class _RestaurentsState extends State<Restaurents> {
  ...
  VendorFilterRequest? currentFilter;
  ...
  Future<void> _openFilterBottomSheet() async {
    final VendorFilterRequest? filter =
        await showModalBottomSheet<VendorFilterRequest>(
          context: context,
          isScrollControlled: true,
          backgroundColor: Colors.transparent,
          builder: (_) => FractionallySizedBox(
    ...
    try {
      final result = await food_Authservice.filterVendors(filter);

      if (!mounted) return;

      setState(() {
        currentFilter = filter;
        _refreshKey++; // This recreates NearbyRestaurentBannersWidget
      });
    } catch (e) {
      if (!mounted) return;
...
class _FilterBottomSheetState extends State<FilterBottomSheet> {
  ...
  bool noPacking = false;
  bool lowPlastic = false;
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            spacing: 12.w,
            runSpacing: 10.h,
            children: [_ratingChip(3.5), _ratingChip(4.0), _ratingChip(4.5)],
          ),
        ],
      ),
    );
  ...
  Widget _ratingChip(double value) {
      ...
      child: Column(
        children: [
          CheckboxListTile(
            ...
            value: isPureVeg,
            onChanged: (v) {
              setState(() => isPureVeg = v!);
            },
          ),

          CheckboxListTile(
            title: const Text("No Packing Charges"),
            value: noPacking,
            onChanged: (v) {
              setState(() => noPacking = v!);
            },
          ),

          CheckboxListTile(
            title: const Text("Low Plastic Packaging"),
            value: lowPlastic,
            onChanged: (v) {
              setState(() => lowPlastic = v!);
            },
          ),
        ],
      ),
      ...
      children: [
        ...
        Expanded(
          child: ElevatedButton(
            onPressed: () {
              final filter = VendorFilterRequest(
                minPrice: minPrice,
                maxPrice: maxPrice,
                ...
                goldOffer: isGold,
                pureVeg: isPureVeg,
                noPackingCharges: noPacking,
                lowPlasticPackaging: lowPlastic,
                orderType: orderType,
                latitude: latitude,
                longitude: longitude,
              );
