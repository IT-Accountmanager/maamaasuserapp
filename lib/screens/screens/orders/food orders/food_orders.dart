...
class _food_ordersState extends State<food_orders> with WidgetsBindingObserver {
  ...
  void dispose() {
    ...
    for (final orderId in _subscribedOrderIds) {
      WebSocketManager().unsubscribeOrderStatus(orderId, listenerId: 'list');
    }
    _subscribedOrderIds.clear();
    super.dispose();
  }
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const Text("No orders yet", style: foodordecolour.h2),
      ...
      child: ListView(
        ...
        children: [
          if (activeOrders.isNotEmpty) ...[
            _sectionLabel("Active Orders", dot: foodordecolour.accent),
  ...
  Widget _sectionLabel(String text, {Color dot = foodordecolour.muted}) {
    return Row(
      children: [
        Container(
          width: 6,
      ...
      MaterialPageRoute(
        builder: (_) => OrderDetailsScreen(
          ...
          formattedDate: DateTimeHelper.formatDate(
            DateTime.utc(
              order.parsedDateTime.year,
              order.parsedDateTime.month,
              order.parsedDateTime.day,
              order.parsedDateTime.hour,
              order.parsedDateTime.minute,
              order.parsedDateTime.second,
              order.parsedDateTime.millisecond,
            ),
          ),
...
class _OrderCardState extends State<OrderCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final bool isAlreadyRated =
        (order.ratings) > 0 ||
        (order.feedback.trim().isNotEmpty) ||
            ...
            child: Column(
              ...
              children: [
                ...
                if (!widget.isActive &&
                    ...
                    !isCancelled) ...[
                  Divider(height: 24.h, color: foodordecolour.border),
  ...
  Widget _buildProgressBar(Order order) {
    ...
    if (order.status == OrderStatus.pending ||
      ...
      return _buildWaitingForConfirmation(order.status);
    }

    // ── Confirmed or beyond → normal stepper ─────────────────────────────
    final steps = [
      ...
      _ProgressStep("on the way", OrderStatus.ontheway),
  ...
  /// Shown instead of the stepper when the restaurant hasn't confirmed yet.
  Widget _buildWaitingForConfirmation(OrderStatus status) {
    final isHold = status == OrderStatus.hold;
    return Container(
      padding: EdgeInsets.symmetric(horizontal: 12.w, vertical: 10.h),
      decoration: BoxDecoration(
        color: foodordecolour.amber.withOpacity(0.08),
        borderRadius: BorderRadius.circular(10.r),
        border: Border.all(
          color: foodordecolour.amber.withOpacity(0.35),
          width: 1,
        ),
      ),
      child: Row(
        children: [
          ...
          Container(
            ...
            height: 32.w,
            decoration: BoxDecoration(
              color: foodordecolour.amber.withOpacity(0.15),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  isHold ? "Order on hold" : "Waiting for restaurant",
                  style: TextStyle(
                    fontSize: 13.sp,
                    fontWeight: FontWeight.w600,
                    color: foodordecolour.ink,
                ...
                Text(
                  isHold
                      ? "The restaurant has put your order on hold."
                      : "To confirm.",
                  style: TextStyle(
                    fontSize: 11.sp,
                    color: foodordecolour.muted,
      ...
      builder: (context, localSetState) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(14.r),
            border: Border.all(color: foodordecolour.border, width: 0.5),
          ),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              ...
              Text(
                "RATE THIS ORDER",
                style: foodordecolour.body.copyWith(
                  fontSize: 11,
                  fontWeight: FontWeight.w600,
                  color: foodordecolour.muted,
                  letterSpacing: 0.8,
              ...
              if (!isSubmitted) ...[
                ...
                DropdownButtonFormField<RatingCategory>(
                  ...
                  decoration: InputDecoration(
                    filled: true,
                    fillColor: foodordecolour.surface,
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    hintStyle: foodordecolour.body.copyWith(
                      fontSize: 14,
                      color: foodordecolour.muted,
                    ),
                    filled: true,
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor: foodordecolour.blue,
              ...
              if (isSubmitted) ...[
                ...
                if (submittedCategory != null)
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: foodordecolour.blue.withOpacity(0.08),
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: foodordecolour.green.withOpacity(0.08),
                    borderRadius: BorderRadius.circular(10.r),
                    border: Border.all(
                      color: foodordecolour.green.withOpacity(0.25),
                      width: 0.5,
                    ),
                  ),
                  child: Row(
  ...
  Widget _buildSubmittedReview(Order order) {
    ...
    final double rawRating = (_submittedRatings[orderId] ?? order.ratings)
        .toDouble();

    final int rating = rawRating.toInt().clamp(0, 5);

    final String feedback = _submittedFeedback[orderId] ?? (order.feedback);
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: foodordecolour.surface,
        borderRadius: BorderRadius.circular(14.r),
        border: Border.all(color: foodordecolour.border.withOpacity(0.5)),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          Divider(color: foodordecolour.border, thickness: 0.5),
...
class _OrderDetailsScreenState extends State<OrderDetailsScreen>
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: foodordecolour.bg,
      appBar: AppBar(
        backgroundColor: foodordecolour.surface,
        elevation: 0,
        ...
        bottom: PreferredSize(
          ...
          child: Divider(height: 1, color: foodordecolour.border),
        ),
      ),
                ...
                children: [
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        // ── Restaurant name ──────────────────────
                        if (order.vendorRegisteredName.trim().isNotEmpty &&
  ...
  Widget _summaryRow(String title, double amount, {Color? color}) {
    ...
    return Padding(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Text(title, style: foodordecolour.body),
          Text(
            "${isNegative ? '-' : ''}₹${amount.abs().toStringAsFixed(2)}",
            style: TextStyle(
              fontSize: 13,
              fontWeight: FontWeight.w500,
              color: color ?? foodordecolour.ink,
            ),
          ),
        ],
      ),
  ...
  Widget _buildDownloadButton(BuildContext context) {
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        style: ElevatedButton.styleFrom(
          backgroundColor: foodordecolour._brandOrange,
          foregroundColor: Colors.white,
          elevation: 0,
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(14),
  ...
  Widget _helpTile({
    ...
    return Container(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          subtitle,
          style: foodordecolour.body.copyWith(fontSize: 12),
        ),
        trailing: const Icon(
          Icons.chevron_right_rounded,
          size: 18,
