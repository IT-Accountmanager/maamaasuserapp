...
//   static const accent = Color(0xFFFF5722); // food-orange accent
...
//
// // ignore: camel_case_types
// class _food_ordersState extends State<food_orders> {
//   bool isLoading = true;
//   List<Order> orders = [];
//
//   @override
...
//       for (var order in orders.where((o) => o.isActive)) {
//         _listenOrderStatus(order.orderId);
//       }
//     } catch (e) {
//       AppAlert.error(context, 'Error loading orders: $e');
//     } finally {
...
//       WebSocketManager().unsubscribeOrderStatus(order.orderId);
//     }
//     super.dispose();
//   }
//
...
//       ),
//     ).then((_) {
//       // Re-subscribe when we come back
//       if (order.isActive) _listenOrderStatus(order.orderId);
//     });
//   }
// }
//
...
//                   ? null
//                   : () async {
//                       setState(() => currentRating = i + 1);
//                       final success = await _submitRating(currentRating);
//                       if (success) {
//                         setState(() => _submittedOrders[orderId] = true);
//                       }
//                     },
//               child: Padding(
...
//                 //       await FoodPdf().downloadInvoice(order.orderId);
//                 //     },
//                 //     child: Container(
//                 //       padding: EdgeInsets.symmetric(
//                 //         horizontal: 10.w,
...
class _food_ordersState extends State<food_orders> with WidgetsBindingObserver {
  ...
  Future<void> _loadOrders() async {
    ...
    try {
      final response = await food_Authservice.getAllOrders();
      final fetchedOrders =
      response.map((json) => Order.fromJson(json)).toList();
      if (!mounted) return;
      setState(() => orders = fetchedOrders);
...
class _OrderCardState extends State<OrderCard> {
  ...
  void initState() {
    super.initState();
    isCancelled = widget.order.status == OrderStatus.cancelled;
    if (!widget.isActive) _fetchRating();
  }
        ...
        Row(
          children: List.generate(steps.length * 2 - 1, (i) {
            if (i.isOdd) {
              final filled = i ~/ 2 < currentIndex;
              return Expanded(
                child: Container(height: 2, color: filled ? _T.accent : _T.border),
              );
            }
            final stepI = i ~/ 2;
            final done = stepI < currentIndex;
