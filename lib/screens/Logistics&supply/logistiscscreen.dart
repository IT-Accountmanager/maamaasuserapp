...
import '../../widgets/recentseraches.dart';
...
class _PassengerFormState extends State<PassengerForm> {
  ...
  bool get canSearch => pickupLocation != null && dropLocation != null;
  ...
  Future<void> _loadCurrentLocation() async {
    ...
    setState(() {
      ...
      pickupController.text = result.fullAddress;
    });
  }

  Future<void> _loadRecentLocations() async {
    _recentLocations = await RecentLocationService.getRecentLocations();
  ...
  Future<void> fetchAvailableVehicles() async {
    ...
    try {
      final vehicles = await DeliveryService.getVehicles(
  ...
  Future<void> _bookRide() async {
    ...
    try {
      await DeliveryService.bookRide(
        userName: _name ?? '',
        userPhone: _mobile ?? '',
  ...
  Widget _vehicleSection() {
    ...
    if (canSearch && availableVehicles.isEmpty) {
      return Padding(
...
class _VehicleCard extends StatelessWidget {
  ...
  String get _emoji {
    switch (vehicle.vehicleType.toUpperCase()) {
      case "BIKE":
        return "🏍️";
